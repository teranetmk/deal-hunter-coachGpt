...
export default {
    name : "ListsPage",
    props: ['allMotivationTags', 'allLists', 'lists', 'personalizedTags', "compaignsData", "basic_list", "pre_list"],
    setup() {
        const date3 = ref();
        ...
        return {date3, responsiveOptions};
    },
    data() {
        return {
            creds_req: 0,
            quantity: 0,
            listName: '',
            basic_list_names: this.basic_list,
            pre_list_names: this.pre_list,
            zipcodeinput: '',
            zipcodeslist: [],
            input_list_name: '',
            tooltip_text: "Data about list",
            basic_display: true,
            pre_display: false,
            inputDate:null,
            filtersList : {
            ...
            displayScanAreaModal      : false,
            displaySelectListModal     :false,
            displayCompaignMessageModal      : false,
            displayListQunatityModal : false,
    ...
    watch : {
        quantity() {
            this.creds_req = this.quantity * 3;
        }
    },
    mounted() {
    ...
    methods   : {
        ...
        async deleteList(listId) {
            const res = await axios.post(route('instalist.delete'), {listId: listId})
        ...
        clearFilter() {
            // this.initFilters();
            this.filtersList.bedmin = '';
            this.filtersList.bedmax = '';

            this.filtersList.bathmax = '';
            this.filtersList.bathmin = '';

            this.filtersList.sqftmin = '';
            this.filtersList.sqftmax = '';

            this.filtersList.yearmin = '';
            this.filtersList.yearmax = '';

            this.filtersList.sdatemin = '';
            this.filtersList.sdatemax = '';
        ...
        changeList(b_list) {
            this.input_list_name = b_list;
            // alert(this.input_list_name)
        },
        deleteObj(zip) {
            let updatedZips = this.zipcodeslist.filter((el) => el.zip !== zip);
            this.zipcodeslist = updatedZips;
        },
        async searchCode() {
            if(this.zipcodeinput.length == 5) {

                    const res = await axios({
                    url    : route('getCodeData'),
                    method : 'POST',
                    data   : {address: this.zipcodeinput},
                    headers: {
                ...
                // this.handleResponse(res.data);
                if (res.data.status === 'success') {
                    this.zipcodeslist.push(res.data.list);
                    this.zipcodeinput = '';
                }
        ...
        listNameStep() {
            if(this.zipcodeslist.length < 1) {
                this.$toast.add({severity: 'error', summary: 'Error', detail: "Select at least one zipcode", life: 3000});
            }
            else {
                this.getprojections();
                this.displayScanAreaModal = false; this.displaySelectListModal = true;
            }
        },
        async getprojections() {
            var data = {codes: this.zipcodeslist,selectedTag: this.input_list_name, listName: this.listName, quantity:this.quantity, skip: this.skipTrace, filters: this.filtersList };
            const res = await axios({
                    url    : route('generateProjections'),
                    method : 'POST',
                    data   : data,
                    headers: {
                        'content-type': 'multipart/form-data'
            ...
            this.basic_list_names = res.data;

        },
        listNamefinal() {
            if(this.input_list_name.length < 1) {
                this.$toast.add({severity: 'error', summary: 'Error', detail: "Select one list", life: 3000});
            }
            else {
                this.displaySelectListModal = false; this.displayListQunatityModal = true;
            }
        },
        async searchList() {
            var data = {codes: this.zipcodeslist,selectedTag: this.input_list_name, listName: this.listName, quantity:this.quantity, skip: this.skipTrace, filters: this.filtersList };
            if(this.listName.length < 1){
                this.$toast.add({severity: 'error', summary: 'Error', detail: "Enter List Name", life: 3000});
            }
            else if(this.quantity < 1){
                this.$toast.add({severity: 'error', summary: 'Error', detail: "Enter quantity", life: 3000});
            }
            else {
                const res = await axios({
                    url    : route('generateList'),
                    method : 'POST',
                    data   : data,
                    headers: {
                        'content-type': 'multipart/form-data'
                ...
                if (res.data.status === 'success') {
                    ...
                    this.displayListQunatityModal = false;
                    this.lists = res.data.lists;
                    this.zipcodeinput = '';
                    this.zipcodeslist = [];
                    this.clearFilter;
    ...
    components: {AuthenticatedLayout, Head, Link,Slider, VueSlider}
}
</script>
<template>
    <AuthenticatedLayout>
        <Head><title>INSTA-LEADS™</title></Head>
        <template #pageHeading>
            <div>
                <!-- <i class="pi pi-book mr-2 text-2xl font-bold"/> -->
                <span class="text-2xl mr-3"><strong> <i class="pi pi-home me-2 text-2xl font-bold"></i> INSTA-LEADS™ Engine </strong></span>
            ...
            <Button class="p-button p-button-outlined p-button-danger"
                    ...
                    icon="pi pi-plus"
                    disabled: true
                    @click="displayScanAreaModal = true"
                    label="Build INSTA-LEADS™ List"></Button>
        </template>
        ...
        <DataTable :value="lists"
            ...
            <Column field="fileName" header="Market Details" sortable style="min-width: 14rem">
                <template #body="{data}">
                    <span>
                    {{ data.filename }}
            ...
            <Column field="properties_count" header="Date Created" sortable style="min-width: 4rem">
                <template #body="{data}">
            ...
            <Column field="tags" header="List Type" sortable style="min-width: 14rem">
                <template #body="{data}">
            ...
            <Column field="import_time" header="Properties" sortable style="min-width: 14rem">
                <template #body="{data}">
            ...
            <Column field="tags" header="ACTION" sortable style="min-width: 14rem">
                <template #body="{data}">
                    ...
                    <Button icon="pi pi-trash"
                            @click="confirmListDelete(data.filename, data.id, data.properties_count)"
    ...
    <Dialog header=""
            :draggable="false"
            v-model:visible="displayScanAreaModal "

            :style="{width: '32vw'}"
            :modal="true">
        <template #header>
            <div class="flex justify-content-between align-items-center " style="width: 90%;">
                <span class="p-dialog-title"><img src="/images/icons/scan.png" /> Choose INSTA-LEADS™ Area</span>
        ...
        <div class="">
            ...
            <div class="field  mt-5">
                <div class="col-12">
                    <span class="p-float-label p-float-label-scan position-relative">
                        <InputText id="zipcode"
                                placeholder="Type Zip Code..."

                                class="w-full py-4 padding-scan-area"
                                type="text"
                                @input="searchCode"
                                style="border-radius: 1rem !important;"
                                v-model="zipcodeinput"/>
                        <label for="inputtext">Search By City Or Zip </label>
                        <i class="pi pi-search search-icon-area"></i>
                        <div
                        class="d-flex filter-btn"
                        @click="displayFilterModal = true"
                        >
                            <img class="mr-2 " src="images/icons/filter.svg"/>
                            <span class="filters-text my-auto">Filters</span>
                        </div>
                        </span>
                </div>
                <div class="d-flex mb-3 ms-2">
                    <div v-for="code in zipcodeslist" :key="code.zip" class="list-button py-2 px-2 mr-2 " >
                        <div class="d-flex" style="align-items: center;">
                            <span class="mr-2 my-auto">{{code.zip}} </span>
                            <span @click="deleteObj(code.zip)" class="pi pi-times my-auto"></span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <Dialog header=""
            :draggable="false"
            v-model:visible="displayFilterModal "

            :style="{width: '666px'}"
            :modal="true">
        <template #header>
            <div class="flex justify-content-between align-items-center " style="width: 90%;">
                <span class="p-dialog-title d-flex">
                    <img class="mr-2" src="/images/icons/filter2.svg" />
                    <span class="my-auto">Narrow Down Your Search</span>
                </span>
            </div>
        </template>
        <div class="filter-box">
            <div class="row mb-3 w-100 m-0">
                <div class="col-md-3">
                    <span class="label-filter my-auto">Bedrooms</span>
                </div>
                <div class="col-md-9">
                    <div class="flex justy-content-space ">
                        <div class="field  ">
                            <div class="w-100">
                                <span class="p-float-label ">
                                    <InputText id="bedmin"
                                            ...
                                            type="text"
                                            style="border-radius: 1rem !important;"
                                            v-model="filtersList.bedmin"/>
                                    <label for="inputtext">Min </label>
                                    </span>
                            </div>
                        </div>

                        <div class="mx-2 d-flex">
                            <span class="m-auto f-divider"></span>
                        </div>
                            ...
                            <div class="w-100">
                                <span class="p-float-label ">
                                    <InputText id="bedmax"
                                            ...
                                            type="text"
                                            style="border-radius: 1rem !important;"
                                            v-model="filtersList.bedmax"/>
                                    <label for="inputtext">Max </label>
                                    </span>
                            </div>
                        </div>
                    </div>
            ...
            <div class="row mb-3 w-100 m-0">
                <div class="col-md-3">
                    <span class="label-filter my-auto">Baths</span>
                </div>
                <div class="col-md-9">
                    <div class="flex justy-content-space ">
                        <div class="field  ">
                            <div class="w-100">
                                <span class="p-float-label ">
                                    <InputText id="bathmin"
                                            class="w-full  "
                                            type="text"
                                            style="border-radius: 1rem !important;"
                                            v-model="filtersList.bathmin"/>
                                    <label for="inputtext">Min </label>
                                    </span>
                            </div>
                        </div>
                        ...
                        <div class="field  ">
                            <div class="w-100">
                                <span class="p-float-label ">
                                    <InputText id="bathmax"
                                            class="w-full  "
                                            type="text"
                                            style="border-radius: 1rem !important;"
                                            v-model="filtersList.bathmax"/>
                                    <label for="inputtext">Max </label>
                                    </span>
                            </div>
                        </div>
            ...
            <div class="row mb-3 w-100 m-0">
                <div class="col-md-3">
                    <span class="label-filter my-auto">Sq. Ft</span>
                </div>
                <div class="col-md-9">
                    <div class="flex justy-content-space ">
                        <div class="field  ">
                            <div class="w-100">
                                <span class="p-float-label ">
                                    <InputText id="sqftmin"
                                            class="w-full  "
                                            type="text"
                                            style="border-radius: 1rem !important;"
                                            v-model="filtersList.sqftmin"/>
                                    <label for="inputtext">Min </label>
                                    </span>
                            </div>
                        </div>
                        ...
                        <div class="field  ">
                            <div class="w-100">
                                <span class="p-float-label ">
                                    <InputText id="sqftmax"
                                            class="w-full  "
                                            type="text"
                                            style="border-radius: 1rem !important;"
                                            v-model="filtersList.sqftmax"/>
                                    <label for="inputtext">Max </label>
                                    </span>
                            </div>
                        </div>
            ...
            <div class="row mb-3 w-100 m-0">
                <div class="col-md-3">
                    <span class="label-filter my-auto">Year Built</span>
                </div>
                <div class="col-md-9">
                    <div class="flex justy-content-space ">
                        <div class="field  ">
                            <div class="w-100">
                                <span class="p-float-label ">
                                    <InputText id="yearmin"
                                            class="w-full  "
                                            type="text"
                                            style="border-radius: 1rem !important;"
                                            v-model="filtersList.yearmin"/>
                                    <label for="inputtext">Min </label>
                                    </span>
                            </div>
                        </div>
                        ...
                        <div class="field  ">
                            <div class="w-100">
                                <span class="p-float-label ">
                                    <InputText id="yearmax"
                                            class="w-full  "
                                            type="text"
                                            style="border-radius: 1rem !important;"
                                            v-model="filtersList.yearmax"/>
                                    <label for="inputtext">Max </label>
                                    </span>
                            </div>
                        </div>
            ...
            <div class="row mb-3 w-100 m-0">
                <div class="col-md-3">
                    <span class="label-filter my-auto">Last Sold Date:</span>
                </div>
                <div class="col-md-9">
                    <div class="flex justy-content-space ">
                        <div class="field  ">
                            <div class="w-100">
                                <span class="p-float-label ">
                                    <InputText id="lastsolddatemin"
                                            class=""
                                            type="text"
                                            placeholder="Min"
                                            onfocus="(this.type='date')" onblur="if(!this.value)this.type='text'"
                                            style="border-radius: 1rem !important; width: 140px;"
                                            v-model="filtersList.sdatemin"
                                            />
                                    <label for="inputtext">Min </label>
                                    </span>
                            </div>
            ...
            <!-- <div class="row mb-3 w-100 m-0">
                <div class="col-md-3">
                    <span class="label-filter my-auto"># Of Units (Multifamily Only)</span>
                </div>
                <div class="col-md-9">
                    <div class="flex justy-content-space ">
                        <div class="field  ">
            ...
            <div class="row mb-3 w-100 m-0">
                <div class="col-md-3">
                    <span class="label-filter my-auto"># Of Properties (Absentee w/ Multiple Only)</span>
                </div>
                <div class="col-md-9">
                    <div class="flex justy-content-space ">
                        <div class="field  ">
            ...
            <div class="row mb-3 w-100 m-0">
                <div class="col-md-3">
                    <span class="label-filter my-auto">Foreclosure Filing Date (Pre-Foreclosure Only)</span>
                </div>
                <div class="col-md-9">
                    <div class="flex justy-content-space ">
                        <div class="field  ">
            ...
            <div class="row px-3">
                ...
                <div class=" col-6 mt-2">
                    <Button class="p-button p-button-outlined p-button-danger w-full py-3"
                            style="border-radius: 0.5rem !important;"
                            @click="filterapply = true; displayFilterModal = false;"
                            >
                            <div class="w-100" style="align-items: center;">
                                <span >Apply Now</span>
    ...
    <!--display seect list modal -->
    <Dialog header=""
            :draggable="false"
            v-model:visible="displaySelectListModal "

            :style="{width: '40vw'}"
            :modal="true">
        <template #header>
            <div class="flex justify-content-between align-items-center " style="width: 90%;">
                <span class="p-dialog-title"><img src="/images/icons/document.svg" /> Choose INSTA-LEADS™ List</span>
            </div>
        </template>
        <div class="">
            <div class="list_types p-0">
                <div class="d-flex p-1 w-100 m-0">
                    <div class="w-50">
                        <Button class="w-100 "
                        :class="basic_display ? 'active' : ''"

                            @click="basic_display = true; pre_display =false;"
                                >
                                <div class="w-100" style="align-items: center;">
                    ...
                    <div class="w-50 ">
                        <Button class="w-100"
                            :class="pre_display ? 'active' : ''"
                            @click="basic_display = false; pre_display =true;"
                                >
                                <div class="w-100" style="align-items: center;">
                                    <span >Premium Lists</span>
            ...
            <div v-if="basic_display == true" class="row w-100 m-0" v-for="b_list in basic_list_names" :key="b_list.id">
                <div class="col-md-9">
                    <div class="basic_list">
                        <div class="mt-3 list_names">
                            <div class="d-flex mb-2" >
                                <div class="form-check my-auto" style="width: 90%;">
                                    <input class="form-check-input" type="radio" name="exampleRadios" @click="changeList(b_list.id)" value="" id="{{b_list.id}}"  >
                                    <label class="form-check-label" for="{{b_list.id}}">
                                        {{b_list.value}} <span v-tooltip.top="b_list.desc " class="my-auto ms-2 help-cir">
                ...
                <div class="col-md-3 d-flex">
                    <span class="my-auto">{{b_list.count}} Leads</span>
                </div>
            </div>

            <div v-if="pre_display == true" class="row w-100 m-0" v-for="b_list in pre_list_names" :key="b_list.id">
                <div class="col-md-9">
                    <div class="basic_list">
                        <div class="mt-3 list_names">
                            <div class="d-flex mb-2" >
                                <div class="form-check my-auto" style="width: 90%;">
                                    ...
                                    <label class="form-check-label" for="{{b_list.id}}">
                                        {{b_list.value}}
                                        <span v-tooltip.top="b_list.desc " class="my-auto ms-2 help-cir">
                                            <img src="/images/icons/help.svg" alt="">
                                        </span>
                                    </label>
                                </div>
                ...
                <!-- <div class="col-md-3 d-flex">
                    <span class="my-auto">254 Leads</span>
                </div> -->
            </div>
     ...
     <Dialog header=""
            :draggable="false"
            v-model:visible="displayListQunatityModal "

            :style="{width: '32vw !important'}"
            :modal="true">
        <template #header>
            <div class="flex justify-content-between align-items-center " style="width: 90%;">
                <span class="p-dialog-title"><img style="width: 19px;" src="/images/icons/diamond.svg" /> Choose INSTA-LEADS™ Quantity</span>
        ...
        <div class="">
                ...
                <div class="col-12">
                    <span class="p-float-label">
                        <InputText id="listName"
                                ...
                                style="border-radius: 1rem !important;"
                                v-model="listName"/>
                        <label for="inputtext">List Name</label>
                        </span>
                </div>
            </div>
                ...
                <div class="col-12">
                    <span class="p-float-label">
                        <InputText  id="quantityList"
                                ...
                                style="border-radius: 1rem !important;"
                                v-model="quantity"/>
                        <label for="inputtext">Lead Quantity</label>
                        </span>
                </div>
            </div>

            <div class="mt-2 field px-3">
                <vue-slider  :height="8"
                    :adsorb="true"
                    :marks="false"
            ...
            <div class="mt-2 d-flex px-3 ">
                <label class="form-check-label mr-2" for="flexSwitchCheckDefault">Skip-Trace Leads?</label>
                <div class="form-check form-switch">

                    <input v-model="skipTrace" class="form-check-input" type="checkbox" id="flexSwitchCheckDefault">
            ...
            <div class="row mx-2 mt-3">
                <div class="col-md-7">
                    <p>
                        The INSTA-LEADS™ campaign will cost {{creds_req}} credits to send
                    </p>
                </div>
                <div class="col-md-5 cred-con d-flex ">
                    <div class="h-100 p-1 mr-2 cred-text">
                        <span class="creditBalance">{{credits.toLocaleString("en-US")}}</span>
                        <br>
                        <span>Credit Balance</span>
